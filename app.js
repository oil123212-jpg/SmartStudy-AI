...
async function loadQuestionSets() {
  ...
  if (!output) return;
  if (!authState.token) {
    output.textContent = "로그인하면 저장된 예상문제 생성 기록이 여기에 표시됩니다.";
    return;
  }
  try {
    ...
    if (!sets.length) {
      output.textContent = "아직 생성된 문제가 없습니다.";
      return;
    }
    renderQuestionSet(sets[0]);
  } catch (error) {
    output.textContent = `문제 목록 불러오기 실패: ${error.message}`;
  }
}
...
if ($("refreshQuestionSetsBtn")) $("refreshQuestionSetsBtn").addEventListener("click", loadQuestionSets);
