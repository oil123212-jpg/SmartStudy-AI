...
def list_question_sets(user_id):
    ...
    with db_connect() as conn:
        conn.row_factory = sqlite3.Row
        rows = conn.execute("SELECT * FROM question_sets WHERE user_id = ? ORDER BY id DESC LIMIT 50", (user_id,)).fetchall()
    return [{
...
def create_question_set(payload, user_id):
    ...
    with db_connect() as conn:
        cur = conn.execute(
            "INSERT INTO question_sets(user_id, subject_id, title, keywords_json, sources_json, questions_json, created_at) VALUES(?, ?, ?, ?, ?, ?, ?)",
            (user_id, subject_id, title, json.dumps(keywords, ensure_ascii=False), json.dumps(sources, ensure_ascii=False), json.dumps(questions, ensure_ascii=False), now),
        )
        conn.commit()
